...
export default function RegisterPage() {
  ...
  const handlePasskeyRegistration = async () => {
    if (!email) {
      setPasskeyError('No email found');
      return;
    }
    setIsRegisteringPasskey(true);
    setPasskeyError(null);
    try {
      const optionsRes = await fetch(`/api/webauthn/options?type=registration&email=${encodeURIComponent(email)}`);
      if (!optionsRes.ok) {
        throw new Error('Failed to get registration options');
      }
      const { publicKey, challengeBase64 } = await optionsRes.json();
      ...
      const credential = await navigator.credentials.create({
        publicKey: {
          ...
          rpId: publicKey.rpId,
          user: {
            id: isoBase64URL.toBuffer(publicKey.user.id),
      ...
      const credentialResponse = {
        ...
        response: {
          attestationObject: isoBase64URL.fromBuffer(response.attestationObject),
          clientDataJSON: isoBase64URL.fromBuffer(response.clientDataJSON),
        },
      };
      ...
      await signIn('credentials', {
        redirect: false,
        email,
        credentialType: 'webauthn-registration',
        webauthnToken: challengeBase64,
    ...
    <div className="fixed inset-0 left-0 right-0 bottom-0 flex items-center justify-center bg-[#FAF6EE] z-40 -mx-4 px-4 pt-24 pb-12 overflow-y-auto">
      <div className="w-full max-w-md space-y-8 animate-fade-up">
        ...
        {!showPasskeyRegistration ? (
          ...
          <div className="relative border-2 border-foreground bg-white p-8 rounded-2xl retro-shadow-md">
            <span className="absolute -top-1.5 -left-1.5 w-2.5 h-2.5 bg-[#4F46E5] border border-foreground" />
            <span className="absolute -top-1.5 -right-1.5 w-2.5 h-2.5 bg-[#4F46E5] border border-foreground" />
            <span className="absolute -bottom-1.5 -left-1.5 w-2.5 h-2.5 bg-[#4F46E5] border border-foreground" />
            <span className="absolute -bottom-1.5 -right-1.5 w-2.5 h-2.5 bg-[#4F46E5] border border-foreground" />
            ...
            <div className="text-center space-y-4">
              ...
              <p className="text-sm text-muted-foreground font-medium">
                Register a passkey for faster and more secure sign-in.
              </p>
              <p className="text-xs font-bold text-foreground bg-[#FAF6EE] border border-foreground/30 rounded-lg px-3 py-2 inline-block">
                ✦ {email}
              </p>
            </div>
            ...
            {passkeyError && (
              <div className="mt-4 border-2 border-[#dc2626] bg-[#fef2f2] rounded-xl p-4 text-sm font-medium text-[#dc2626]">
                {passkeyError}
              </div>
            )}
          </div>
